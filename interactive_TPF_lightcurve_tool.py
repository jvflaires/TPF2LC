...
class TPF2LC:
    ...
    def __init__(self, tpf):
        ...
        self.data = (np.nansum(self.tpf.flux, axis=0) / np.median(np.nansum(self.tpf.flux, axis=0))).value # 2D flux normalized data
        ...
        self.allowed_backends = ['qtagg']
        self.check_backend()

    
    # Function to detect the matplotlib backend used
    def check_backend(self):
        """
        Check the matplotlib backend used and returns an error if the 
        backend used is not QtAgg.
        """
        backend = matplotlib.get_backend()
        if backend not in self.allowed_backends:
            raise RuntimeError(f"QtAgg backend recommended for full interactivity.")
    ...
    # Function to update pixel square with mouse movement
    def on_mouse_move(self, event):
        """
        Reads mouse position and updates the rectangle around the pixels.
        """
        if event.inaxes:
    ...
    # Function that allows pixel selection with mouse event
    def on_click(self, event):
        """
        Reads the position of mouse click, select the corresponding pixel and 
        updates the light curve.
        """
        if event.inaxes:
            if event.inaxes == self.ax1:
                x, y = int(round(event.xdata)), int(round(event.ydata))
                if (x, y) not in self.marked_pixels:
                    self.marked_pixels.append((x, y))
                else:
                    self.marked_pixels.remove((x, y))
                self.update_hashed_pixels()
                self.selected_pixels = np.zeros(self.data.shape, dtype='bool')
                for (x, y) in self.marked_pixels:
                    self.selected_pixels[y,x] = True
    ...
    def get_mask(self):
        ...
        x,y = zip(*self.marked_pixels) if self.marked_pixels else ([],[])
        mask[np.array(y), np.array(x)] = True
        return mask
    ...
    def interact(self):
        ...
        # Connect to allow key and mouse events
